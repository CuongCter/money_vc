...
import { Home, CreditCard, BarChart2, Settings, X, TrendingUp } from "lucide-react"
...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const navItems = [
    ...
    { name: "Ngân sách", path: "/budget", icon: TrendingUp },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-white border-r border-gray-200 transition-all duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
      ...
      >
        <div className="flex items-center justify-between h-16 px-6 border-b border-gray-200">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="p-2 rounded-md text-gray-500 hover:text-gray-900 hover:bg-gray-100 focus:outline-none lg:hidden transition-colors duration-200"
          >
            <X size={20} />
          </button>
        </div>
